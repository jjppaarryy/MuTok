...
export async function GET(
  ...
  try {
    ...
    const plan = await prisma.postPlan.findUnique({ where: { id } });
    if (!plan?.renderPath) {
      return new Response("Not found", { status: 404 });
    }

    // Validate the file path is within the allowed data directory
    const safePath = validateFilePath(plan.renderPath);
    if (!safePath) {
      console.warn(`Blocked access to path outside data directory: ${plan.renderPath}`);
      return new Response("Not found", { status: 404 });
    }
